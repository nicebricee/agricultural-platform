...
class SupabaseClient:
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            response = self.client.table('state_agricultural_metrics').select('*').limit(1).execute()
            return True
        except Exception as e:
        ...
        try:
            ...
            async with httpx.AsyncClient() as http_client:
                ...
                return {
                    ...
                    "row_count": len(response.json()) if isinstance(response.json(), list) else 1
                }
    ...
    async def _execute_table_query(self, query: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            query_builder = self.client.table('state_agricultural_metrics').select('*')
...
class Neo4jClient:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.driver = AsyncGraphDatabase.driver(
                ...
                max_connection_pool_size=10,  # Reduced to avoid too many connections
                connection_timeout=30,
                max_transaction_retry_time=30,
                keep_alive=True,  # Enable keep-alive
                connection_acquisition_timeout=60  # Wait longer for available connection
            )
    ...
    async def execute_query(self, query: str, params: Dict[str, Any] = None, preserve_graph_structure: bool = True) -> Dict[str, Any]:
        ...
        if not self.driver:
            ...
            app_logger.warning("Neo4j driver not initialized, attempting to reconnect...")
        ...
        for attempt in range(max_retries):
            try:
                ...
                async with self.driver.session(database=self.database) as session:
                            ...
                            for key, value in record.items():
                                ...
                                if isinstance(value, Node):
                                    ...
                                    if node_id not in graph_data["nodes"]:
                                        ...
                                        app_logger.debug(f"Found node: {node_id} with labels {list(value.labels)}")
                                # Check if value is a Relationship
                                    ...
                                    graph_data["relationships"].append({
                                        "id": rel_id,
                                        "type": value.type,
                                        "start": f"n:{value.start_node.element_id if hasattr(value.start_node, 'element_id') else value.start_node.id}",
                                        "end": f"n:{value.end_node.element_id if hasattr(value.end_node, 'element_id') else value.end_node.id}",
                                        "properties": dict(value.items())
                                    })
                                    app_logger.debug(f"Found relationship: {rel_id} of type {value.type}")
                                # Check if value is a Path
                                    ...
                                    app_logger.debug(f"Found path with {len(value.nodes)} nodes")
                    ...
                    if preserve_graph_structure and (graph_data["nodes"] or graph_data["relationships"]):
                ...
                if "defunct connection" in error_msg:
                    ...
                    app_logger.warning(f"Defunct connection detected (attempt {attempt + 1}/{max_retries}), recreating driver...")
                    if self.driver:
                        await self.driver.close()
                        self.driver = None
                    await asyncio.sleep(retry_delay)
                    if not await self.initialize():
                        app_logger.error("Failed to reinitialize Neo4j driver after defunct connection")
                elif attempt < max_retries - 1:
                    app_logger.warning(f"Neo4j connection error (attempt {attempt + 1}/{max_retries}): {e}")
                    await asyncio.sleep(retry_delay * (2 ** attempt))  # Exponential backoff
                    # Try to reinitialize the driver
                    await self.initialize()
                else:
                    app_logger.error(f"Neo4j query failed after {max_retries} attempts: {e}")
                    return {
                        "data": [],
                        "execution_time": 0,
                        "row_count": 0,
...
class DatabaseManager:
    ...
    async def execute_parallel_queries(
        ...
        cypher_params: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """Execute SQL and Cypher queries in parallel."""
        app_logger.debug(f"Executing parallel queries:\nSQL: {sql_query}\nCypher: {cypher_query}")
        ...
        try:
            ...
            return {
                ...
                "total_execution_time": sql_result.get("execution_time", 0) + graph_result.get("execution_time", 0)
            }
